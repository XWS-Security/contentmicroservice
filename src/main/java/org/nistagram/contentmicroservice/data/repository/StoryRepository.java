...
public interface StoryRepository extends CrudRepository<Story, Long> {
    ...
    @Query(value = "SELECT c.id, c.tags, c.date, c.location_id, c.path, c.content_type, c.path, c.only_close_friends, c.highlights" +
            " FROM content AS c, user_content AS uc" +
            " WHERE c.content_type = 'STORY' AND uc.user_id = :id AND c.id = uc.content_id", nativeQuery = true)
